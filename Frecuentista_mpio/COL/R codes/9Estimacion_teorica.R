...
encuesta_MC <- inner_join(encuesta_MC, 
  ...
  temp <- poststrat_temp %>% 
    mutate(depto,n, 
           ...
           Z2 = ifelse( (delta - k) <= alpha &  (delta - k) > 0,
                        prob_educacion*(1 - prob_empleo) + 
                        prob_empleo*(1 - prob_educacion)  +
                          prob_educacion*prob_empleo  
                          , 0),
           
           Z3 = ifelse( (delta - k) <= 2*alpha &  (delta - k) > alpha, 
                        prob_educacion*prob_empleo,
                               0),
           
           Z4 = ifelse( (delta - k) > 2*alpha , 0, 0),
           Ez = n*(Z1+Z2+Z3+Z4)) %>% 
