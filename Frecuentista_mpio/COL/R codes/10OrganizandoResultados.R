...
readRDS(file = "Frecuentista_mpio/COL/Data/ipm_educacion.rds") %>% 
  rename(sae_MC_Educacion = ipm_educacion_estimado_MC),
readRDS(file = "Frecuentista_mpio/COL/Data/ipm_empleo.rds")%>% 
  rename(sae_MC_Empleo = ipm_empleo_estimado_MC)) %>% 
full_join(
    readRDS(file = "Frecuentista_mpio/COL/Data/ipm_MC.rds")%>% 
...
# smce empleo 
smce_empleo <- 
list.files("Frecuentista_mpio/COL/Data/iter_empleo_BOOT_MC/",
           full.names = TRUE) %>% 
  map_df(~readRDS(.x) ) %>% 
  mutate(diff = ipm_empleo_estimado_MC - ipm_boot) %>% 
  group_by(mpio) %>% summarise(smce_empleo = sqrt(mean(diff^2)))
# smce educacion 
smce_educacion <- 
  list.files("Frecuentista_mpio/COL/Data/iter_educacion_BOOT_MC/",
             full.names = TRUE ) %>% 
  map_df(~readRDS(.x) ) %>% 
  mutate(diff = ipm_educacion_estimado_MC - ipm_boot) %>% 
  group_by(mpio) %>% summarise(smce_educacion = sqrt(mean(diff^2)))
# smce ipm 
smce_ipm <- 
  list.files("Frecuentista_mpio/COL/Data/iter_IPM_BOOT_MC/",
             full.names = TRUE) %>% 
  map_df(~readRDS(.x) ) %>% 
  mutate(diff = ipm_boot_estimado - ipm_boot) %>% 
  group_by(mpio) %>% summarise(smce_IPM = sqrt(mean(diff^2)))
...
png(filename = "Frecuentista_mpio/COL/Output/Comparando_dir_censo_sae/Educacion.png")
plot(Estimacion$Dir_Educacion, Estimacion$sae_MC_Educacion)
abline(b=1,a=0, col = "red")
dev.off()

png(filename = "Frecuentista_mpio/COL/Output/Comparando_dir_censo_sae/Empleo.png")
plot(Estimacion$Dir_Empleo, Estimacion$sae_MC_Empleo)
abline(b=1,a=0, col = "red")
dev.off()

png(filename = "Frecuentista_mpio/COL/Output/Comparando_dir_censo_sae/Ipm.png")
plot(Estimacion$Dir_IPM, Estimacion$sae_MC_IPM)
abline(b=1,a=0, col = "red")
dev.off()


Estimacion %>% select(mpio,nd, Dir_Empleo, sae_MC_Empleo) %>% 
  gather(key = variable, value = "value",-mpio,-nd) %>% 
ggplot(data = .) +
  geom_jitter(aes(
    x = fct_reorder2(mpio, mpio, nd),
    y = value,
    color = variable
  ), size = 2.5,width = 0.3) +
  scale_color_manual(
...
Estimacion %>% select(mpio,nd, Dir_Educacion, sae_MC_Educacion) %>% 
  gather(key = variable, value = "value",-mpio,-nd) %>% 
  ggplot(data = .) +
  geom_jitter(aes(
    x = fct_reorder2(mpio, mpio, nd),
...
Estimacion %>% select(mpio,nd, Dir_IPM, sae_MC_IPM) %>% 
  gather(key = variable, value = "value",-mpio,-nd) %>% 
  ggplot(data = .) +
  geom_jitter(aes(
    x = fct_reorder2(mpio, mpio, nd),
