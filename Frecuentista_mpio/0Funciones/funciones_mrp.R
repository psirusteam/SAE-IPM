...
if(all(any(byMap != "mpio"))) {
  ...
  ## Completando registros
    th <- paste0(
    "dat_df %<>%tidyr::expand(mpio ,", fnames,",", cnames,
    ") %>% full_join(dat_df, by = c('mpio','", fnames,"' , '", cnames ,"' ))"
  )
  eval(parse(text = th))

paso <- Shape %>% left_join(dat_df,  by = c("mpio")) %>%
    group_by_at(all_of(byMap)) %>%
    group_nest() %>%
    filter_at(all_of(c(fnames,cnames)),any_vars(!is.na(.)))
